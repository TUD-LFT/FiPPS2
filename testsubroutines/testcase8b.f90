...
!
subroutine testcase8b (testcase)
...
double precision,parameter	:: fakelemdicke=0.005d0
...
if (anzelemprohalbbreite < anzelemprohalblaenge) then
   elemdicke=fakelemdicke*(2*anzelemprohalbbreite*2)	! 2. mal 2 wegen Symmetrie->platte eigentlich doppelt so breit
else
   elemdicke=fakelemdicke*(2*anzelemprohalblaenge*2)
end if
...
!   forces(ii)%nid = anzknoten-(ii-1)
!   forces(ii)%cid = 0
!   if (ii==anzknotenlaenge) then
!      forces(ii)%fac = 5.D0!*(elemdicke**3)
!   else
!      forces(ii)%fac = 1.D0*200.D0!*(elemdicke**3)
!   end if
!   forces(ii)%ni  = (/ 0.D0, 0.D0, -0.25D0 /)
...
do ii=1,anzknoten
   ...
   forces(ii)%cid = 0
!   if (ii==anzknotenlaenge) then
!      forces(ii)%fac = 0.5D0!*(elemdicke**3)
!   else
