...
!
subroutine testcase13 (testcase)
...
allocate(loads(2*anzknotenlaenge+2*anzknotenbreite))

do ii=1,(2*anzknotenlaenge+2*anzknotenbreite)
...
allocate(forces(2*anzknotenlaenge+2*anzknotenbreite))

!write(*,*) 'anzforces: ', 2*anzknotenlaenge+2*anzknotenbreite
num = 1
...
do ii=1,anzknotenlaenge
   ...
   forces(num)%lid = num
   forces(num)%nid = anzknoten-(ii-1)
   forces(num)%cid = 0
   
   if (ii==1 .OR. ii==anzknotenlaenge) then
   
      forces(num)%fac = 1.D0/6.D0*force
         
   else
      
      if (mod(ii,2) > 0) then
	 forces(num)%fac = 1.D0/3.D0*force
      else
         forces(num)%fac = 2.D0/3.D0*force
      end if
      
   end if
   forces(num)%ni  = (/ 1.D0, 0.D0, 0.D0 /)
   
!   write(*,*) 'num: ', num
!   write(*,*) 'nodenum: ', forces(num)%nid
!   write(*,*) 'forces(num)%fac: ',forces(num)%fac
...
!   write(*,*) 'nodenum: ', nodenum
   ...
   forces(num)%ni  = (/ 0.D0, -1.D0, 0.D0 /)
   
!   write(*,*) 'num: ', num
!   write(*,*) 'forces(num)%fac: ',forces(num)%fac
   
   if (mod(ii,2) > 0) then
      nodenum = nodenum+anzknotenbreite
   else
      nodenum = nodenum+int(dble(anzknotenbreite)/2.D0+0.5D0)
   end if
