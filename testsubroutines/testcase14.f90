...
!
subroutine testcase14 (testcase)
...
double precision, parameter	:: elemlaenge = 0.1D0
double precision, parameter	:: elembreite = 0.1D0
...
allocate(spcds(anzknotenlaenge))

do ii=1,anzknotenlaenge
  
   spcds(ii)%sid = anzspc1+ii
   spcds(ii)%nid = anzknoten-(ii-1)
   spcds(ii)%dof = 2
   spcds(ii)%val = 1.D0
...
allocate(spcadds(anzspc1+anzknotenlaenge))

do ii=1,anzspc1+anzknotenlaenge
