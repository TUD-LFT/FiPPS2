...
!
subroutine testcase10b (testcase)
...
integer,parameter		:: rbx0=12345, rbxlaenge=5					       ! RB an den Seiten (ohne Eckknoten)
integer,parameter		:: rby0=12345, rbybreite=4
integer,parameter		:: rbx0y0=12345, rbxlaengey0=12345, rbx0ybreite=12345, rbxlaengeybreite=45	! Eckknoten
integer 			:: countrb, temp, facI, count_dof

! SS-SS

!integer,parameter		 :: rbx0=3, rbxlaenge=5 					! RB an den Seiten (ohne Eckknoten)
!integer,parameter		 :: rby0=23, rbybreite=4
!integer,parameter		 :: rbx0y0=123, rbxlaengey0=235, rbx0ybreite=34, rbxlaengeybreite=45	 ! Eckknoten
!integer			 :: countrb, temp, facI, count_dof
...
if (anzelemlaenge*elemlaenge < anzelembreite*elembreite) then
   elemdicke=fakelemdicke*(2*anzelemlaenge*elemlaenge)
else
   elemdicke=fakelemdicke*(2*anzelembreite*elembreite)
end if
...
!      forces(ii)%fac = 1.D0/6.D0*10.D0
!      forces(ii)%fac = 5.D0
!   else if (ii==2 .OR. ii==(anzknotenlaenge-1)) then
!      forces(ii)%fac = 2.D0/3.D0*10.D0
!      forces(ii)%fac = 10.D0
!   else
!      forces(ii)%fac = 10.D0
!   end if
!   forces(ii)%fac = 1.D0*(elemdicke**3)
!   forces(ii)%ni  = (/ 0.D0, 0.D0, -0.01D0 /)
...
do ii=1,anzknotenbreite
   do jj=1,anzknotenlaenge
      ...
      forces(num)%lid = num
!      forces(num)%nid = anzknoten-(jj-1)-ii*anzknotenlaenge
      forces(num)%nid = anzknoten-num+1
      forces(num)%cid = 0
	 ...
	 if (jj==1) then
	    ...
	    forces(num)%fac = -0.001666666666D0/2.D0*(elemdicke**3)
	    ...
	    if (mod(jj,2) > 0) then
	       ...
	       forces(num)%fac = -0.001666666666D0*(elemdicke**3)
	       
	       num = num+1
	       
	    else
	       
	       forces(num)%fac =  0.003333333333D0*(elemdicke**3)
         ...
         if (mod(ii,2) > 0) then   ! ungerade Zeile
	    ...
	    if (jj == 1) then
	       ...
	       num = num+1
    	       ...
    	       if (mod(jj,2) > 0) then
    	    	  ...
    	    	  forces(num)%fac = -0.003333333333D0*(elemdicke**3)
!   	    	  forces(num)%fac = 0.006666666666D0
    	    	  
    	    	  num = num+1
    	    	  
    	       else
    	    	  
    	    	  forces(num)%fac =  0.006666666666D0*(elemdicke**3)
!   	    	  forces(num)%fac = 0.003333333333D0
...
!   	    	  forces(num)%fac = 0.003333333333D0
      ...
      forces(num-1)%ni  = (/ 0.D0, 0.D0, 1.D0 /)
...
if (num == anzknoten) then
   write(*,*) 'japps'
else
   write(*,*) 'nope'
   write(*,*) 'anzknoten: ', anzknoten
   write(*,*) 'num: ', num
end if
!stop
!allocate(loads(1))
...
!forces(2)%cid = 0
!forces(2)%fac = 10.D0
!forces(2)%ni  = (/ 0.D0, 1.D0, 0.D0 /)
!
!forces(3)%lid = 1
!forces(3)%nid = 6
!forces(3)%cid = 0
!forces(3)%fac = 5.D0
!forces(3)%ni  = (/ 0.D0, 1.D0, 0.D0 /)
