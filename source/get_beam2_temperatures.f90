...
!
! =================================================================================================
subroutine get_beam2_temperatures(fesim,lcID, beam2_temperatures)
! =================================================================================================
! use
!
...
!
! Output
double precision, dimension(size(fesim%elemente%beam2s,1)), intent(out)    :: beam2_temperatures    !< Temperatures on each node for this load case
!
! Internal
!
integer                                :: ii, jj
...
!
 ...
 do ii = 1,size(fesim%lasten%beam2temps,1)
   do jj = 1,size(fesim%lasten%loads,1)
     if ((fesim%lasten%loads(jj)%lcid .eq. lcID) .and. (fesim%lasten%loads(jj)%lidi .eq. fesim%lasten%beam2temps(ii)%lid) .and. (fesim%lasten%beam2temps(ii)%eid .eq. 0)) then
       T_ref = fesim%lasten%beam2temps(ii)%temp
     end if
   end do
 end do
 ...
 do ii=1,size(fesim%lasten%beam2temps,1)
   do jj = 1,size(fesim%lasten%loads,1)
     if((fesim%lasten%loads(jj)%lcid .eq. lcID) .and. (fesim%lasten%loads(jj)%lidi .eq. fesim%lasten%beam2temps(ii)%lid) .and. (fesim%lasten%beam2temps(ii)%eid .ne. 0)) then
       ! Assign temperature to element
       beam2_temperatures(fesim%lasten%beam2temps(ii)%eid) = fesim%lasten%beam2temps(ii)%temp - T_ref
     end if
   end do
 end do
!
...
end subroutine get_beam2_temperatures
