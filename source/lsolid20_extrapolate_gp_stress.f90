...
!
!> @brief
!> Extrapolate stresses from 2x2 gauss points in a xi-eta-plane to the corner nodes
!> of the same plane; optional transformation to global coordinate system
!
!> @details
!> Stresses are extrapolated from the 2x2 gauss points in a xi-eta-plane to the corner
!> nodes of the same plane; optionally, the resulting stresses are transformed to the
!> element or material coordinate system; input stresses must be given in the element
...
!
!> $Id: lsolid20_extrapolate_gp_stress.f90 484 2024-10-18 14:28:29Z s1080304 $
!> $Author: s1080304 $
!> $Revision: 484 $
!> $Date: 2024-10-18 16:28:29 +0200 (Fr, 18. Okt 2024) $
!
! =================================================================================================
subroutine lsolid20_extrapolate_gp_stress(fesim,node_coords,zeta,cid,phi,outCS,stresses_gp,stresses_cn)
! =================================================================================================
! use
!
...
double precision, intent(in), dimension(6,4)    :: stresses_gp !< (6,4)-array containing the stresses at the plane's 2x2 gauss points with respect to the element coordinate system and the following ordering: s = [s11, s22, s33, s23, s13, s12]^T
!
! Output
!
double precision, intent(out), dimension(6,4)   :: stresses_cn !< (6,4)-array containing the stresses at the plane's corner nodes with respect to the specified output coordinate system
!
! Internal
!
integer                                         :: err_code=0
...
double precision, dimension(6,4)                :: stresses_temp
...
! Calculation
 ...
 do ii = 1,6
   stresses_temp(ii,:) = matmul(EXT,stresses_gp(ii,:))
 end do

 if (outCS .EQ. 'elem') then
  
   ! stresses are already in element coordinate system
   stresses_cn(:,:) = stresses_temp(:,:)
   ...
   ! transform strain
   stresses_cn = matmul(transpose(T),stresses_temp) 

 else if (outCS .EQ. 'glob') then 
   ! ---------------------------------------------------------------------------
   ! Begin Retransformation of stresses from element to global coordinate system
   ...
   do ii = 1,4
     ...
     ! Transform vector of stresses from
     ! element to global coordinate system
     call transform_eg_stress(fesim,stresses_temp(:,ii),E,cid,stresses_cn(:,ii),'eg')
   end do
   ! End Transformation of stresses from element to gobal coordinate system
   ! ---------------------------------------------------------------------------
 else
...
if (err_code /= 0) then
   ...
   write(*,*)                      'lsolid20_extrapolate_gp_stress'
...
end subroutine lsolid20_extrapolate_gp_stress
