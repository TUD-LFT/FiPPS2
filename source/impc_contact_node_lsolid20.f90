...
!
!> @brief
!> convertion of the Node-LSOLID20-Contacts to MPCs
!
!> @details
!
!> @author
...
!> 21.04.2022
!
!> $Id: impc_contact_node_quad8.f90 414 2019-10-17 11:45:47Z DOM+ahauffe $
!> $Author: DOM+ahauffe $
!> $Revision: 414 $
!> $Date: 2019-10-17 13:45:47 +0200 (Do, 17. Okt 2019) $
!
! =================================================================================================
subroutine impc_contact_node_lsolid20 (fesim,ind_offset)
!
...
integer                                 :: ii,jj,kk,mm
integer                                 :: lsolid20ID, nodeii
double precision, dimension(20,3)       :: node_coords
double precision, dimension(3)          :: xk, nc
double precision                        :: xi,eta,zeta
double precision, dimension(20)         :: Ni
integer                                 :: err_code=0
double precision, dimension(3,3,20)     :: transMat
logical, dimension(20)                  :: nodeHasLocalCoord
integer                                 :: numMPCTerms
integer                                 :: iMPC
integer                                 :: nid

integer, parameter                      :: numMpcPerContact = 3
  
  do ii = 1, size(fesim%randbedingungen%contact_node_lsolid20, 1)
  
    lsolid20ID = fesim%randbedingungen%contact_node_lsolid20(ii)%lsolid20ID
    
    numMPCTerms = 0

    do kk=1,20
    
      nid = fesim%elemente%lsolid20s(lsolid20ID)%nids(kk)
    ...
    do nodeii = 1, size(fesim%randbedingungen%contact_node_lsolid20(ii)%nodeIDs,1)
    
      xk(1:3) = fesim%knoten%nodes(fesim%randbedingungen%contact_node_lsolid20(ii)%nodeIDs(nodeii))%coords(1:3)
  
      ! Bestimmen der natuerlichen Koordinaten des zu kontaktierenden Knotens
      call impc_contact_lsolid20_naturalcoordinates(node_coords, xk, xi, eta, zeta)

      call hexa20_shapefunctions_xi_eta_zeta(xi, eta, zeta, Ni=Ni)

      ! MPC fuer die drei Translationen
      do jj = 1,3
        fesim%randbedingungen%mpcs(ind_offset+numMpcPerContact*(nodeii-1)+jj-1)%mpc_type = 1
        fesim%randbedingungen%mpcs(ind_offset+numMpcPerContact*(nodeii-1)+jj-1)%dependend%nid = fesim%randbedingungen%contact_node_lsolid20(ii)%nodeIDs(nodeii)
      ...
      do kk = 1,20
        if (nodeHasLocalCoord(kk) .EQ. .FALSE.) then
          do jj = 1,3
            fesim%randbedingungen%mpcs(ind_offset+numMpcPerContact*(nodeii-1)+jj-1)%independend(iMPC)%nid = fesim%elemente%lsolid20s(lsolid20ID)%nids(kk)
          ...
          do jj = 1,3
            do mm = 1,3
              fesim%randbedingungen%mpcs(ind_offset+numMpcPerContact*(nodeii-1)+jj-1)%independend(iMPC+mm-1)%nid = fesim%elemente%lsolid20s(lsolid20ID)%nids(kk)
    ...
    ind_offset = ind_offset + numMpcPerContact * size(fesim%randbedingungen%contact_node_lsolid20(ii)%nodeIDs,1)
    
    deallocate(fesim%randbedingungen%contact_node_lsolid20(ii)%nodeIDs)
    
  end do
  
  deallocate(fesim%randbedingungen%contact_node_lsolid20)
...
if (err_code /= 0) then
   ...
   write(*,*)                      'impc_contact_node_lsolid20'
...
end subroutine impc_contact_node_lsolid20
