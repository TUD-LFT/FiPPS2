...
!
subroutine quad8_principle_values(voigtVec, fac, princVal)

  implicit none
  
  double precision, dimension(6), intent(in)  :: voigtVec
  double precision, intent(in)                :: fac
  double precision, dimension(3), intent(out) :: princVal
  
  double precision, dimension(3,3)            :: tensor
  
  tensor(1,1) = voigtVec(1); tensor(1,2) = fac*voigtVec(4); tensor(1,3) = fac*voigtVec(6)
                             tensor(2,2) =     voigtVec(2); tensor(2,3) = fac*voigtVec(5)
                                                            tensor(3,3) =     voigtVec(3)

  CALL DSYEVC3(tensor, princVal)
...
end subroutine quad8_principle_values
